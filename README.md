...
**Hidden**:
The Island draws nothing at rest; the Notch looks like a bare notch. It still responds to hover. The Island is Hidden when there is no Now Playing, or Now Playing has been paused for 3 minutes or more.
_Avoid_: idle, collapsed, closed
...
**Expanded**:
The full panel dropped down from the Notch. Entered by hovering the Notch while Hidden or the middle of Peek; exited when the pointer leaves it. Shows Now Playing (art, title, artist, playback progress) with the transport controls (previous, play/pause, next) when Now Playing exists, otherwise shows the Pins. The artwork opens the song and the artist name opens an artist search in Spotify.
